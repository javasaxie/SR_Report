...
function loadPurchasing() {
  console.log("loadPurchasing called");
...
function renderTable(data) {
  ...
  if (data) {
    data.forEach(x => {
      html += `<tr><td>${x.date}</td><td>${x.item}</td><td>${x.part}</td><td>${x.merk}</td><td>${x.vendor}</td><td>${x.qty}</td><td>${formatMoney(x.total)}</td><td><button class="btn btn-danger btn-sm" onclick="deletePurchase('${x.id}')"><i class="bi bi-trash"></i></button></td></tr>`;
    });
  }
...
function filterPurchase() {
  let key = document.getElementById("searchPurchase").value.toLowerCase();
